...
計算式スロットで数字をピックする新感覚ビンゴゲーム。QRコードでスマホから参加して、リーチ・ビンゴを自己申告できる。
...
* 各要素は5回ランダム表示してから確定
* 各要素の確定間隔: 環境変数 `SLOT_ANIMATION_DELAY` で設定（デフォルト500ms）
* 全体の流れ:
  1. 左の数字確定 → 待機
  2. 演算子確定 → 待機
  3. 右の数字確定 → 待機
...
```bash
# スロットアニメーションの待機時間（ミリ秒）
SLOT_ANIMATION_DELAY=500  # デフォルト値
```
